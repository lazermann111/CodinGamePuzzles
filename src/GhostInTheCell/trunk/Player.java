...
class Player {
    ...
    public void Run()
    {
        ...
        while (true)
        {
            ...
            if(Tick == 0) MainBasesDistance = InitMainBasesDistance(StongestFactory(OwnerType.ME), StongestFactory(OwnerType.OPPONENT));
            if(Tick == 0 || Tick == 10) SendBombs(res);
            Factory myStrongest = StongestFactory(OwnerType.ME);
    ...
    {
        return DistanceToFactory(f1.Id, f2.Id);
    }
    ...
    private void DefendBases(StringBuilder res)
    {
        ...
        {
            List<Troops> myTroops = IncomingTroops(f.Id, OwnerType.ME);
            List<Troops> oppTroops = IncomingTroops(f.Id, OwnerType.OPPONENT);

            boolean needToDefend = myTroops.stream().mapToInt(a -> a.Amount).sum() + f.CyborgsAmount =< oppTroops.stream().mapToInt(a -> a.Amount).sum();
            if(needToDefend)
            {
                Factory nearest = NearestFactory(f.Id, OwnerType.ME, 0);
    ...
    public List<Factory> MyFactories()
    {
    ...
    public List<Troops> IncomingTroops(int destinationId, OwnerType ownerType)
    {
