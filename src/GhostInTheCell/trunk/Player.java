...
import static GhostInTheCell.trunk.Helpers.AddFactory;
import static GhostInTheCell.trunk.Helpers.AddTroops;
import static GhostInTheCell.trunk.Helpers.DistanceToFactory;
import static GhostInTheCell.trunk.Helpers.IncomingTroops;
import static GhostInTheCell.trunk.Helpers.MyFactories;
import static GhostInTheCell.trunk.Helpers.NearestFactory;
import static GhostInTheCell.trunk.Helpers.StongestFactory;
...
class Player {
	...
	public void Run() {
		...
		while (true) {
			...
			for (int i = 0; i < entityCount; i++) {
				...
				switch (entityType) {
					case "FACTORY":
						AddFactory(factories, entityId, arg1, arg2, arg3);
						break;
					case "TROOP":
						AddTroops(troops, entityId, arg1, arg2, arg3, arg4, arg5);
						break;
					case "BOMB":
						break;
					default:
			...
			if (Tick == 0) {
				MainBasesDistance =
						InitMainBasesDistance(StongestFactory(factories, OwnerType.ME), StongestFactory(factories, OwnerType.OPPONENT));
			}
			if (Tick == 0 || Tick == 10) {
				SendBombs(res);
			}
	...
	private void DefendBases(StringBuilder res) {
		for (Factory f : MyFactories(factories)) {
			...
			boolean needToDefend =
					myTroops.stream().mapToInt(a -> a.Amount).sum() + f.CyborgsAmount <= oppTroops.stream().mapToInt(a -> a.Amount).sum();
			if (needToDefend) {
				Factory nearest = NearestFactory(factories, distances, f.Id, OwnerType.ME, 0);
				if (nearest != null) {
					res.append(nearest.MoveTo(f.Id, 2));
				}
			}
		}
	}
